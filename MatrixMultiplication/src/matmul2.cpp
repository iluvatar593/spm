...
//			}
			...
			matrixSum(A, offsetA, &A[newsize], offsetA+newsize, (NUM **) S1, newsize); //S1 has been calculated
			matrixSum(B, offsetB, &B[newsize], offsetB+newsize, (NUM **) S2, newsize); //S2 has been calculated
			//printf("Until now it seems ok!\n");
			NUM P1[newsize][newsize];
			//for(int i = 0; i < newsize; i++) P1[i] = new NUM[newsize]();

			strassenMatrixMultiplication((NUM **) S1, (NUM **) S2, (NUM **) P1, newsize); //P1 = S1*S2
			//printf("Until now it seems ok!\n");
			//Fin qui tutto bene
			matrixSum(&A[newsize], offsetA, &A[newsize], offsetA+newsize, (NUM **) S1, newsize); //S3
			...
			strassenMatrixMultiplication(B, (NUM **) S1, (NUM **) P2, newsize); //P2 calculated
			//printf("Until now it seems ok!\n");
			//printf("P2 calculated \n"); //fin qui tutto bene
			matrixSub(B, offsetB+newsize, &B[newsize], offsetB+newsize, (NUM **) S2, newsize); //S4
			//if(S2[0][0] >= 0) printf("Error in S4\n");
			//printf("S4 calculated \n");
			NUM P3[newsize][newsize];
			//for(int i = 0;i < newsize; i++) P3[i] = new NUM[newsize]();
			//printf("Until now it seems ok!\n");
			strassenMatrixMultiplication(A, (NUM **) S2, (NUM **) P3, newsize); //P3 calculated, check fi space can be reused
			//printf("Until now it seems ok!\n");
			//	printf("P3 calculated \n");
			matrixSub(&B[newsize], offsetB, B, offsetB, (NUM **) S1, newsize); //S5

			NUM P4[newsize][newsize];
			//for(int i = 0;i < newsize; i++) P4[i] = new NUM[newsize]();
			normalMatrixMultiplication(&A[newsize], (NUM **) S1, (NUM **) P4, newsize, newsize);
			//printf("Until now it seems ok!\n");
			//if(P4[0][0] >= 0) printf("Error in P4 %d\n", P4[0][0]);
			matrixSum(A, offsetA, A, offsetA+newsize, (NUM **) S2, newsize); //S6
			//printf("Until now it seems ok!\n");
			//if(S2[0][0] != 1) printf("Error in S6 %d\n", S2[0][0]);
			NUM P5[newsize][newsize];
			//for(int i = 0;i < newsize; i++) P5[i] = new NUM[newsize]();
			normalMatrixMultiplication((NUM **) S2, &B[newsize], (NUM **) P5, newsize, 0, newsize);
			//printf("Until now it seems ok!\n");
			//if(P5[0][0] != 1) printf("Error in P5 %d\n", P5[0][0]);
			matrixSub(&A[newsize], offsetA, A, offsetA, (NUM **) S1, newsize); //S7
			//if(S1[0][0] >= 0) printf("Error in S7 %d\n", S1[0][0]);
			matrixSum(B, offsetB, B, offsetB+newsize, (NUM **) S2, newsize); //S8
			//if(S2[0][0] != 1) printf("Error in S8 %d\n", S2[0][0]);
			NUM P6[newsize][newsize];
			//for(int i = 0;i < newsize; i++) P6[i] = new NUM[newsize]();
			strassenMatrixMultiplication((NUM **) S1, (NUM **) S2, (NUM **) P6, newsize);
			//if(P6[0][0] >= 0) printf("Error in P6 %d\n", P6[0][0]);
			matrixSub(A, offsetA+newsize, &A[newsize], offsetA+newsize, (NUM **) S1, newsize); //S9
			//if(S1[0][0] >= 0) printf("Error in S9%d\n", S1[0][0]);
			matrixSum(&B[newsize], offsetB, &B[newsize], offsetB+newsize, (NUM **) S2, newsize); //S10
			//if(S2[0][0] != 1) printf("Error in s10 %d \n", S2[0][0]);
			NUM P7[newsize][newsize];
			//for(int i = 0; i < newsize; i++) P7[i] = new NUM[newsize]();
			strassenMatrixMultiplication((NUM **) S1, (NUM **) S2, (NUM **) P7,newsize);
