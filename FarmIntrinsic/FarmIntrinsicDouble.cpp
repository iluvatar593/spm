...
int main(int argc, const char** argv) {
	...
	if (k > TILE*31) {
		if(isatty(fileno(stdout)))printf("Sorry, k is too large! Try again with a value smaller or equal to %d\n", TILE*31);
		printUsage();
		return -1;
	}
	if(k%TILE > 0) {
	...
	_MM_MALLOC(aBuffer, double**, sizeof(double*)*bufferSize);
	_MM_MALLOC(bBuffer, double**, sizeof(double*)*bufferSize);
	srand(time(NULL));
	/** Initialization of the whole input buffer */
	for(int i = 0; i < bufferSize; i++) {
		/** Allocate & initialize a couple of matrices */
		_MM_MALLOC(aBuffer[i], double*, sizeof(double)*n*k);
		initializeDoubleMatrix(aBuffer[i], n, k);
		_MM_MALLOC(bBuffer[i], double*, sizeof(double)*k*m);
		initializeDoubleMatrix(bBuffer[i], k, m);
	...
	for(int i = 0; i < numWorkers; i++) w.push_back(new WorkerDouble(n, oldn, k, oldk, m, oldm));
	farm->add_workers(w);
	farm->run_and_wait_end();
	printf("Farm\t Double \t %d \t %d \t %d \t %d \t %d \t %f\n", streamLength, numWorkers, n, k, m, farm->ffTime());
